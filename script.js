...
L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", { maxZoom: 7 }).addTo(map);

let issIcon = L.icon({ iconUrl: "https://upload.wikimedia.org/wikipedia/commons/d/d0/International_Space_Station.svg", iconSize: [50, 32] });
...
let altitudeChart = new Chart(ctx, {
  type: "line",
  data: { labels: [], datasets: [{ label: "ISS Altitude (km)", data: [], borderWidth: 2, fill: false }] },
  options: {
    responsive: true,
    scales: { x: { title: { display: true, text: "Time" } }, y: { title: { display: true, text: "Altitude (km)" } } },
  },
});
